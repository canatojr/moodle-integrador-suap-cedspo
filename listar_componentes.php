...
<table class="table">
  <thead>
    <tr><th>ID</th><th>Período</th><th>Sigla</th><th>Descrição</th><th>Qtd. avaliações</th></tr>
  </thead>
  <tbody>
    <?php
    foreach (ComponenteCurricular::ler_rest($id_curso) as $row):
      $periodo = $row->optativo ? 'Optativo' : "$row->periodo&ordm;";
      echo "<tr><td>{$row->id}</td><td>{$periodo}</td><td>{$row->sigla}</td><td>{$row->descricao}</td><td>{$row->qtd_avaliacoes}</td></tr>";
    endforeach
    ?>
  </tbody>
</table>
